...
class User {
  ...
  async createUser(
    ...
    try {
      ...
      if (role == "trainee") {
        result = await pool.query(
          `INSERT INTO trainee VALUES ($1, $2, $3, $4, $5)`,
          [id, activity, height, weight, goal]
        );
      }
  ...
  async getInfo() {
    const result = await pool.query(
      `SELECT user_id, name, password, email, age, gender, user_role
                                     FROM users 
                                     WHERE user_id = $1`,
      [this.id]
    );
  ...
  async editinfo(name, password, email, age, gender) {
    const result = await pool.query(
      `UPDATE users
                                     ...
                                     WHERE user_id = $1`,
      [this.id, name, password, email, age, gender]
    );
  ...
  async update_trainer(trainer_id) {
    var result = await pool.query(
      `update trainer set trainer_id = $2 where trainee_id = $1`,
      [this.id, trainer_id]
    );
    if (result.rowCount == 0) {
      result = await pool.query(`INSERT into trainer values ($2, $1)`, [
        this.id,
        trainer_id,
      ]);
    }
  ...
  async update_dietician(dietician_id) {
    var result = await pool.query(
      `update dietician set dietician_id = $2 where trainee_id = $1`,
      [this.id, dietician_id]
    );
    if (result.rowCount == 0) {
      result = await pool.query(`INSERT into dietician values ($2, $1)`, [
        this.id,
        dietician_id,
      ]);
    }
